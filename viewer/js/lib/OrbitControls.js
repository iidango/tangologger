...
THREE.OrbitControls = function ( object, domElement ) {
    ...
    // 	var offset = pixelToVertex(cam, shot, 0, 0, 1);
    // 	this.animationTarget.copy(offset);
    // };
    this.goto_shot = function(cam, shot, change_direction = false) {
        let oc = opticalCenter(shot);
        if (cam.projection_type == 'equirectangular') {
            if (change_direction){
                let dir = vector3To2(oc).sub(vector3To2(this.animationPosition)).angle();
                if(dir < Math.PI * 1.0/4.0){
                    naviDirection.set(0.1, 0, naviDirection_z);
                }else if(dir < Math.PI * 3.0/4.0){
                    naviDirection.set(0, 0.1, naviDirection_z);
                }else if(dir < Math.PI * 5.0/4.0){
                    naviDirection.set(-0.1, 0, naviDirection_z);
                }else if(dir < Math.PI * 7.0/4.0){
                    naviDirection.set(0, -0.1, naviDirection_z);
                }else{
                    naviDirection.set(0.1, 0, 0);
                }
                console.log(dir / (Math.PI * 2) * 360);
                this.animationTarget.copy(oc).add(naviDirection);

            }else{
                this.animationTarget.add(oc).sub(this.animationPosition);
            }
            // this.animationTarget.add(oc).sub(this.animationPosition);
        ...
        this.animationPosition.copy(oc).add(naviPosOffset);
    };
    // this.goto_shot = function(cam, shot) {
    // 	if (cam.projection_type == 'equirectangular') {
    ...
    this.change_direction = function(rad){
        // rotating across whole screen goes 360 degrees around
        scope.rotateLeft( rad );
    }

    this.change_top = function(rad){
        // rotating across whole screen goes 360 degrees around
        scope.rotateUp( rad );
    }
