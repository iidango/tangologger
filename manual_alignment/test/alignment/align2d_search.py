...
def on_floor_height(z, floor_height, height):
    return z - floor_height >= 0 and z - floor_height < height

def fetch_on_floor_shots(tra_dict, center, height):
    shot_list = []
    for k in tra_dict:
        if on_floor_height(tra_dict[k][2], center - height/2, height):
            shot_list.append(k)
    return shot_list


def find_floors(tra_dict, height):
    z_list = []
    for k in tra_dict:
        z_list.append(tra_dict[k][2])
    
    # prepare for clustering
    z_list = np.array(z_list)
    z_list = np.c_[z_list, np.zeros(z_list.shape[0])]    # expand axis

    for clu_num in range(1, 6):
        km = KMeans(n_clusters=clu_num)
        km.fit(z_list)
        centers = km.cluster_centers_[:, 0]
        centers.sort()

        floor_heights = [centers[i + 1] - centers[i] for i in range(clu_num-1)]
        shot_nums = [len(fetch_on_floor_shots(tra_dict, centers[i], height)) for i in range(clu_num)]
        if clu_num == 1:
            c = centers[0]
            floor_candidates = centers
        if clu_num != 1 and all([height > 15 for height in floor_heights]) and all([n > 100 for n in shot_nums]) :
            floor_candidates = []
            floor_candidates = centers
            # print(clu_num, centers, shot_nums)

    floor_candidates.sort()
    floor_candidates = [math.floor(h - height/2 + 0.5) for h in floor_candidates]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--merge', default=False, action='store_true', help='run_merge(default=False)')
    ...
    for data_name in target_pairs:
        ...
        # find floor heights
        floor_heights = find_floors(tra_dict, config.align_voxel_size[2])
        for h in floor_heights:
            logger.info('floor heights: {}'.format(h))
        ...
        for floor in target_pairs[data_name]:
            ...
            # print(floor_heights)
            logger.info('total num: {}x{}x{}={}'.format(len(range_x), len(range_y), len(floor_heights), len(range_x)*len(range_y)*len(floor_heights)))

            ## load score map from csv
            logger.info('loading score map from csv...')
            score_map_dict = {}
            ...
            results = [[] for z in range(len(floor_heights))]

            def calc_score(arg):
                tra_y = arg[0]
                tra_x = arg[1]
                result = [None for z in range(len(floor_heights))]
                ...
                count_yxz = np.zeros((fp_img.shape[0]//config.align_voxel_size[1], fp_img.shape[1]//config.align_voxel_size[0], len(floor_heights)), dtype=int)
                hit_voxel_count = [0 for z in range(len(floor_heights))]
                hit_shot_z_list = [[] for z in range(len(floor_heights))]
                for shot_name in hit_shot_list:
                    ...
                    for z, floor_height in enumerate(floor_heights):
                        if on_floor_height(xyz[2], floor_height, config.align_voxel_size[2]):
                            # print(xyz[2], floor_height, shot_name)
                ...
                for z in range(len(floor_heights)):
                    if hit_voxel_count[z] < config.floor_voxel_count_threshold:
                        continue

                    # print('z: ', z, ', len: ', len(hit_shot_z_list[z]), ', hit_shot_len: ', len(hit_shot_list), 'not_hit_shot_len', len(not_hit_shot_list))
                    score = 0.0
                    ...
                    sub_result['z'] = - int(floor_heights[z])
            ...
            for z, floor_height in enumerate(floor_heights):
                ...
                if len(results[z]) == 0:
                    continue
                logger.info('max score in z={:3d} is {:.8f}({}, {})(in {} candidates)'.format(-floor_height, results[z][0]['score'], results[z][0]['x'], results[z][0]['y'], len(results[z])))
            ...
            if config.calc_correct:
                ...
                if not os.path.exists(tra_floor_fn) or not os.path.exists(tra_fn):
                    ...
                    tra_y /= c
                    tra_z /= c
                    tra_z += config.align_voxel_size[2]/2
                    tra_x, tra_y, tra_z = int(tra_x), int(tra_y), int(tra_z)
                    # print('actual tranclation ', tra_x, tra_y, tra_z)
                    ...
                    for r in actual:
                        if r is not None:
                            logger.debug('actual result cand: {} ({}, {}, {})'.format(r['score'], r['x'], r['y'], r['z']))

                    # get actual z 
                    min_diff = 1000000000000000
                    act_z = None
                    for z, floor_height in enumerate(floor_heights):
                        diff = abs(tra_z + floor_height)
                        if diff < min_diff:
                            min_diff = diff
                            act_z = z
                        # print(z, floor_height, diff)
                    if actual[act_z] is None:
                        logger.warning('actual score not found: {} - {}'.format(data_name, floor))
                    else:
                        logger.info('actual score on {}: {:.8f} ({}, {}, {})'.format(floor, actual[act_z]['score'], actual[act_z]['x'], actual[act_z]['y'], actual[act_z]['z']))
                        actual = [actual[act_z]]
                        actual_result[floor] = actual

            data = {'align_result': align_result, 'actual_result': actual_result, 'range_x': range_x, 'range_y': range_y, 'range_z': None, 'floor_heights': floor_heights}
