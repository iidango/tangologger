...
def floorplan_from_json(key, obj):
    """
    Read a floorplan from a json object
    """
    ...
    metadata = types.FloorplanMetadata()
    metadata.pix_per_meter = obj["pix_per_meter"]
    metadata.width = obj["width"]
    metadata.height = obj["height"]
...
def reconstruction_from_json(obj):
    ...
    if 'metadata' in obj:
        ...
        if "translation" in obj['metadata']['offset']:
            pose.translation = obj['metadata']['offset']["translation"]
        metadata.offset = pose

        pose = types.Pose()
        pose.rotation = obj['metadata']['shots_offset']["rotation"]
        if "translation" in obj['metadata']['shots_offset']:
            pose.translation = obj['metadata']['shots_offset']["translation"]
        metadata.shots_offset = pose

        pose = types.Pose()
        pose.rotation = obj['metadata']['points_offset']["rotation"]
        if "translation" in obj['metadata']['points_offset']:
            pose.translation = obj['metadata']['points_offset']["translation"]
        metadata.points_offset = pose
        pose = types.Pose()

        pose.rotation = obj['metadata']['floorplans_offset']["rotation"]
        if "translation" in obj['metadata']['floorplans_offset']:
            pose.translation = obj['metadata']['floorplans_offset']["translation"]
        metadata.floorplans_offset = pose

        reconstruction.metadata = metadata
...
def floorplan_to_json(floorplan):
    """
    Write a floorplan to a json object
    """
    obj = {
        'rotation': list(floorplan.pose.rotation),
        'translation': list(floorplan.pose.translation),
    }
    if floorplan.metadata.pix_per_meter is not None:
        obj['pix_per_meter'] = floorplan.metadata.pix_per_meter
    if floorplan.metadata.width is not None:
        obj['width'] = floorplan.metadata.width
    if floorplan.metadata.height is not None:
        obj['height'] = floorplan.metadata.height

    return obj

def metadata_to_json(metadata):
    """
    Write a metadata to a json object
    """
    obj = {}

    obj['offset'] = {
        'rotation': list(metadata.offset.rotation),
        'translation': list(metadata.offset.translation),
    }
    obj['shots_offset'] = {
        'rotation': list(metadata.shots_offset.rotation),
        'translation': list(metadata.shots_offset.translation),
    }
    obj['points_offset'] = {
        'rotation': list(metadata.points_offset.rotation),
        'translation': list(metadata.points_offset.translation),
    }
    obj['floorplans_offset'] = {
        'rotation': list(metadata.floorplans_offset.rotation),
        'translation': list(metadata.floorplans_offset.translation),
    }
...
def reconstruction_to_json(reconstruction):
    ...
    # Extract floorplans
    for floorplan in reconstruction.floorplans.values():
        obj['floorplans'][floorplan.id] = floorplan_to_json(floorplan)

    # Extract metadata
    obj['metadata'] = metadata_to_json(reconstruction.metadata)
